...
def reflectance_direction(view_dir, normal):
    # correct shading view direction, already checked.
    shading_view_direction = -view_dir
    inner_prod = (shading_view_direction * normal).sum(dim=-1)
    inner_prod = torch.clamp(inner_prod, min=0.)[..., None]
    mask = (shading_view_direction * normal).sum(dim=-1) < 0.0
    reflection_dir = 2 * inner_prod * normal - shading_view_direction
    reflection_dir[mask] = 0.0
    return reflection_dir

class NeuralRefRenderer(nn.Module):
    def __init__(self, params):
        super(NeuralRefRenderer, self).__init__()
        self.params = params
        ...
        self.spatial_enc = tcnn.Encoding(3, encoding_config)
        ...
        self.texture_dec = tcnn.Network(self.params.hash_n_levels * self.params.hash_n_features_per_level + 16, 3, network_config)
    ...
    def forward(self, sample, verts, faces, normals, centers, scales,):
        """
        :param imgs: [B, H, W, 3]
        :param c2ws: [B, 4, 4]
        :param cpos: [B, 3]
        ...
        spatial_feat = self.spatial_enc(frg_xyz)
        
        frg_ref = reflectance_direction(frg_dir, frg_normal)
        ref_feat = self.dir_encoder(frg_ref.reshape(-1, 3) * 0.5 + 0.5)
        
        neural_colour = self.texture_dec(torch.cat([spatial_feat, ref_feat], dim=-1))
